// original src: https://github.com/MilanKovacic/vite-plugin-externalize-dependencies/blob/main/src/index.ts
// just adapted to use rolldownOptions for vite 8
 ...
 * This plugin is injected into optimizeDeps.rolldownOptions.plugins, and runs during the dependency scanning / optimization phase.
 *
...
const vitePluginExternalize = (options: PluginOptions): Plugin => ({
  ...
  config: (config: UserConfig): Omit<UserConfig, 'plugins'> | null | void => {
    ...
    config.optimizeDeps.rolldownOptions.plugins ??= [] // eslint-disable-line no-param-reassign

    // Prevent the plugin from being inserted multiple times
    const pluginName = 'externalize'
    const isPluginAdded = config.optimizeDeps.rolldownOptions.plugins.some(
